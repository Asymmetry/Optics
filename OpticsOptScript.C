...
void DoMinTP(TString SourceDataBase, TString DestDataBase, Int_t MaxDataPerGroup = 200)
{
    // minimize with root
    ...
    opt->LoadRawData(DataSource, 2000000, MaxDataPerGroup);
    opt->PrepareSieve();
    ...
    Int_t i = 1;
    TCanvas* c;
    while (c = static_cast<TCanvas*> (next())) c->Print(DestDataBase + Form(".Sieve.Opt.%d.png", i++), "png");

    delete fitter;
}

void DoMinYt(TString SourceDataBase, TString DestDataBase, Int_t MaxDataPerGroup = 200)
{
    // minimize with root
    ...
    Int_t i = 1;
    TCanvas* c;
    while (c = static_cast<TCanvas*> (next())) c->Print(DestDataBase + Form(".Yt.Opt.%d.png", i++), "png");

    delete fitter;
}

void DoMinDp(TString SourceDataBase, TString DestDataBase, Int_t MaxDataPerGroup = 200)
{
    // minimize with root
    ...
    Int_t i = 1;
    TCanvas* c;
    while (c = static_cast<TCanvas*> (next())) c->Print(DestDataBase + Form(".Dp.Opt.%d.png", i++), "png");

    delete fitter;
}

void PlotDataBase(TString DatabaseFileName, Int_t MaxDataPerGroup = 200)
{
    opt = new LOpticsOpt();
...
void LOpticsOptScript(TString select, TString SourceDataBase, TString DestDataBase)
{
    ...
    case 3:
        ...
        DoMinYt(SourceDataBase, DestDataBase, 100);
        break;
    case 4:
        ...
        DoMinDp(SourceDataBase, DestDataBase, 1000);
        break;
    case 10:
